...
class TweetDataset(Dataset):
    ...
    def __init__(self, tweets, targets, tokenizer, max_len, e2v, w2v, method):
        self.tweets = tweets
    ...
    def __getitem__(self, item):
        tweet = method(str(self.tweets[item]))
...
def create_data_loader(X, y, tokenizer, max_len, batch_size, e2v, w2v, method):
    ds = TweetDataset(
        tweets=X,
        targets=y,
        tokenizer=tokenizer,
...
if __name__ == "__main__":
    ...
    if method_case == 'rm':
        ...
        raise Exception(
            'Invaild method to incorporating emojis. Available methods are: rm, dir, emoji2desc, concat-desc')
    ...
    for seed in range(5):
        ...
        train_data_loader = create_data_loader(X_train, y_train, TweetTknzr, MAX_LEN, BATCH_SIZE, e2v, w2v, method)
        val_data_loader = create_data_loader(X_val, y_val, TweetTknzr, MAX_LEN, BATCH_SIZE, e2v, w2v, method)
        test_data_loader = create_data_loader(X_test, y_test, TweetTknzr,  MAX_LEN, BATCH_SIZE, e2v, w2v, method)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_acc > best_accuracy:
                torch.save(decoder.state_dict(), f'{method}-e2v-{seed}.pt')
                best_accuracy = val_acc

            decoder.load_state_dict(torch.load(f'{method}-e2v-{seed}.pt'))
        ...
        test_acc_list.append(test_acc.cpu().item())
    print(test_acc_list)